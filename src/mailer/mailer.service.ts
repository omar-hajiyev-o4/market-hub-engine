...
export class MailerService {
  ...
  async sendMail(subject: string, html: string): Promise<void> {
    ...
    if (!process.env.EMAIL_USER || !process.env.EMAIL_PASSWORD) {
      this.logger.warn(`Skipping email to ${to}: EMAIL_USER or EMAIL_PASSWORD is missing in .env`);
      return;
    }
    ...
    try {
      ...
      this.logger.error(`SMTP Error: Failed to send email to ${to}. If using Gmail, ensure you are using an "App Password" rather than your main password!`);
