...
export class ScraperService {
  ...
  async websiteScrapingMain(sourceSite: string = 'irshad.az'): Promise<any> {
    ...
    if (configError || !configData) {
      addLog('error', `Failed to fetch config for ${sourceSite}`, configError);
    ...
    const processCategory = async (category: any) => {
      ...
      while (hasMore) {
        try {
          addLog('log', `[Worker] Kategoriya: ${category.name} | Döngü ${loopCounter} başladı. DOM oxunur...`);

          // Send config to Puppeteer context
          const extractedProducts = await page.evaluate((evalConfig) => {
            const results: any[] = [];
          const productCards = document.querySelectorAll(evalConfig.selectors.productCard);

          productCards.forEach(card => {
            const colorInputs = card.querySelectorAll(evalConfig.selectors.colorInputs);
            const priceEl = card.querySelector(evalConfig.selectors.priceCurrent) as HTMLElement;
            const oldPriceEl = card.querySelector(evalConfig.selectors.priceOld) as HTMLElement;
            const labelEl = card.querySelector(evalConfig.selectors.label) as HTMLElement;

            const currentPrice = priceEl ? parseFloat(priceEl.innerText.replace(/[^0-9.]/g, '')) : 0;
            const oldPrice = oldPriceEl ? parseFloat(oldPriceEl.innerText.replace(/[^0-9.]/g, '')) : null;

            let inStock = true;
            if (labelEl) {
              const labelText = labelEl.innerText.trim().toLowerCase();
              const keywordsArray = evalConfig.out_of_stock_keywords || [];
              const isOutOfStock = keywordsArray.some((keyword: string) => labelText.includes(keyword));
              if (isOutOfStock) {
                inStock = false;
              }
            }
            ...
            if (colorInputs && colorInputs.length > 0) {
              colorInputs.forEach((input: Element) => {
                results.push({
                  title: input.getAttribute(evalConfig.attributes.colorTitle),
                  product_url: input.getAttribute(evalConfig.attributes.colorUrl),
                  main_image_url: input.getAttribute(evalConfig.attributes.colorImage),
                  price_current: currentPrice,
                  price_old: oldPrice,
                  is_in_stock: inStock,
                  has_colors: true,
                  parent_title: card.querySelector(evalConfig.selectors.parentTitle)?.textContent?.trim() || 'Məhsul'
                });
              });
            } else {
              const defaultLink = card.querySelector(evalConfig.selectors.defaultLink) as HTMLAnchorElement;
              const defaultImg = card.querySelector(evalConfig.selectors.defaultImage) as HTMLImageElement;

              results.push({
                title: defaultLink ? defaultLink.innerText.trim() : 'Ad tapılmadı',
        ...
        addLog('log', `[Worker] Kategoriya: ${category.name} | Bu döngüdə ${extractedProducts.length} məhsul tapıldı.`);
        ...
        if (extractedProducts.length > 0) {
          ...
          addLog('log', `[Worker] ${category.name} - Supabase products_tmp-ə ${mappedData.length} məhsul əlavə edilir...`);
          const { error: upsertError } = await supabase.from('products_tmp').upsert(mappedData, { onConflict: 'product_url' });
          if (upsertError) {
            addLog('error', `[Worker] ${category.name} - Supabase yazılmasında xəta:`, upsertError);
          } else {
            addLog('log', `[Worker] ${category.name} - Batch uğurla Supabase-ə yazıldı.`);
          }
        }
        ...
        try {
          ...
          if (loadMoreBtn) {
            ...
            if (isVisible) {
              addLog('log', `[Worker] ${category.name} - Daha çox yüklə düyməsi basılır. Səhifənin yüklənməsi gözlənilir...`);
              
              // Click via evaluate to avoid Puppeteer scrolling/hanging issues if the element is covered or detached
              await page.evaluate(el => (el as HTMLElement).click(), loadMoreBtn);
              ...
              try {
                await page.waitForSelector(config.selectors.productCard, { timeout: 10000 });
                // Small artificial delay to allow the rest of the batch to finish rendering
                await new Promise(resolve => setTimeout(resolve, 2000));
                
                loopCounter++;
              } catch (waitError) {
                addLog('warn', `[Worker] ${category.name} - Yeni məhsullar yüklənmədi (Timeout). Kategoriya ola bilsin ki bitib.`);
                hasMore = false;
              }
            } else {
              addLog('log', `[Worker] ${category.name} - Daha çox yüklə düyməsi görünmür. Pagination bitdi.`);
              hasMore = false;
            }
          } else {
            addLog('log', `[Worker] ${category.name} - Daha çox yüklə düyməsi tapılmadı. Pagination bitdi.`);
            hasMore = false;
          }
        } catch (err) {
          addLog('warn', `[Worker] ${category.name} - Sonrakı səhifəyə keçərkən xəta:`, err);
          hasMore = false;
        }
      } catch (loopError) {
        addLog('error', `[Worker] Kategoriya ${category.name} üçün loop xətası:`, loopError);
        hasMore = false;
      }
      }
    ...
    const promises = categories.map((category: any) => limit(() => processCategory(category)));
    const results = await Promise.all(promises);
    
    totalScraped = results.reduce((acc, curr) => acc + curr, 0);

    await browser.close();
    addLog('log', `Proses uğurla bitdi! Çəkilən ümumi məhsul sayı: ${totalScraped}`);
    ...
    await this.syncDatabase(sourceSite);
      ...
      statusCode = 500;
      errorDump = err instanceof Error ? (err.stack ?? err.message) : JSON.stringify(err);
  ...
  async syncDatabase(sourceSite: string): Promise<void> {
    ...
    const { error } = await this.supabaseService.getClient().rpc('sync_products', { p_source_site: sourceSite });
